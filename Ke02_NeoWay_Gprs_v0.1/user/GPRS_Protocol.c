...
{	
	...
	if((1==flag)||(0!=Master_Inf.RunningTime))  //²»ÔÚÔËÐÐ¶¼ÎªÎ´Ê¹ÓÃ
	{
		flag=1;
...
uint8  Gprs_ReceiveEvent(void)
{
	...
	if(1==Gprs_flag.bits.Deal_data)			//Êý¾Ý½ÓÊÕÍê±Ï£¬¿ªÊ¼´¦ÀíÊý¾Ý
	{  
	...
	*(source+1)=ACK_CRC_FAULT;
	temp_crc = crc16(source,length-2);//-2Îª¿Û³ýÔ­CRCµÄ³¤¶È
...
void Init_Gprs_Device(void)
{
  ...
  uint8 temp_ip_3[4]  =SERVER_IP_3;
	...
	{
		Device_Info.Serial_Id[i] = temp_id[i];   //Éè±¸ID
	}    
  
	for(i=0;i<SERVER_IP_LENGTH;i++)
	{
		Web_Param.Server_Ip_1[i] = temp_ip_1[i];   //·þÎñÆ÷IP
	}
	for(i=0;i<SERVER_PORT_LENGTH;i++)
	{
  	...
  	for(i=0;i<SERVER_IP_LENGTH;i++)
	{
		Web_Param.Server_Ip_2[i] = temp_ip_2[i];   //·þÎñÆ÷IP
	}
	for(i=0;i<SERVER_PORT_LENGTH;i++)
	{
		Web_Param.Server_Port_2[i] = temp_port_2[i];//·þÎñÆ÷¶Ë¿ÚºÅ
	}
  for(i=0;i<SERVER_IP_LENGTH;i++)
	{
		Web_Param.Server_Ip_3[i] = temp_ip_3[i];   //·þÎñÆ÷IP
	}
	for(i=0;i<SERVER_PORT_LENGTH;i++)
	{
		Web_Param.Server_Port_3[i] = temp_port_3[i];//·þÎñÆ÷¶Ë¿ÚºÅ
	}
